        ...
        if (gameState == GameState.Playing || gameState == GameState.WaitingLose) {
            ...
            if (p >= minPercent) {
                ...
                if (SaveData.VibrationEnabled == 1)
                    Handheld.Vibrate();
                MissionManager.GetInstance().UpdateMissionProgress(MissionType.FinishOverBallCount, ballCount);
            }
            
            MissionManager.GetInstance().UpdateMissionProgress(MissionType.DestroyBarrels, 1);
        }
    }
